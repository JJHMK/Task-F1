...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "Tiedot puuttuvat tai ovat virheellisiä. Tarkista lomakkeen kentät ja yritä uudelleen."; 
  }

  const lines = errors.map((e) => {
    const field = e.field || "kenttä";
    const msg = e.msg || "Virheellinen arvo";
    return `• ${field}: ${msg}`;
  });

  return `Lomakkeen tiedoissa on korjattavaa. Tarkistathan seuraavat kentät:\n\n${lines.join("\n")}`;
}
...
function buildGenericErrorMessage(status, body) {
  ...
  const error = body?.error ? body.error : "Pyyntö epäonnistui";
  return `Palvelin palautti virheen (${status}).\n\nSyy: ${error}${details}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg = "Nimi on jo käytössä. Valitse toinen nimi tai tarkista jo luodut resurssit listasta.";
        showFormMessage("info", `Resurssin luonti estetty ⚠️\n\n${msg}`); 
        return;
      }
    ...
    // Ystävällinen viesti ensin
    const resourceName = body?.data?.name ?? "Resurssi";
    msgLines.push(`Hienoa! "${resourceName}" lisättiin onnistuneesti! ✅\n`);
    
    // Alkuperäiset tarkat tiedot alle
    ...
    showFormMessage("error", "Verkkovirhe: Palvelimeen ei saatu yhteyttä. Tarkista nettiyhteys ja yritä uudelleen.");
  }
}